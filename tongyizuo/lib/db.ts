...
import type { ClusterResponse, ClusterToStore, EnrichmentResult } from './types';

// ─── Read ────────────────────────────────────────────────────────────────────

export async function getWordCluster(simplified: string): Promise<ClusterResponse | null> {
  // Find all word rows for this simplified character (may have multiple pronunciations)
  ...
  // Use the first row as the representative word for the response header
  const word = wordRows.rows[0];
  const wordIds = wordRows.rows.map((r) => r.id);

  // Find all clusters any pronunciation of this word belongs to
...
// ─── Enrich ───────────────────────────────────────────────────────────────────
// Called lazily when a cluster exists in DB but hasn't been enriched yet.

export async function storeEnrichment(
  clusterId: number,
  enrichment: EnrichmentResult,
): Promise<void> {
  // Fetch word IDs for this cluster
  const memberRows = await sql`
    SELECT w.id, w.simplified
    FROM words w
    JOIN cluster_members cm ON cm.word_id = w.id
  ...
  for (const row of memberRows.rows) {
    wordIdMap[row.simplified] = row.id;
  }

  // Update core_scene for each word
  for (const [simplified, data] of Object.entries(enrichment.words)) {
    const wordId = wordIdMap[simplified];
    if (!wordId) continue;
    await sql`
      UPDATE words SET core_scene = ${data.core_scene}
      WHERE id = ${wordId} AND core_scene IS NULL
    `;
  }

  // Insert collocations + sharing
  for (const [simplified, data] of Object.entries(enrichment.words)) {
    const wordId = wordIdMap[simplified];
    if (!wordId || !data.collocations) continue;

    for (const coll of data.collocations) {
      const result = await sql`
        INSERT INTO collocations (word_id, collocation, pinyin, gloss, weight, pattern_type)
        VALUES (${wordId}, ${coll.phrase}, ${coll.pinyin}, ${coll.gloss}, ${coll.weight}, ${coll.pattern})
        ON CONFLICT (word_id, collocation) DO UPDATE SET
          weight = EXCLUDED.weight,
          pinyin = EXCLUDED.pinyin,
          gloss = EXCLUDED.gloss
  ...
  // Insert situations
  if (enrichment.situations) {
    for (const sit of enrichment.situations) {
      const answerWordId = wordIdMap[sit.answer];
      if (!answerWordId) continue;
      await sql`
        INSERT INTO situations (cluster_id, answer_word_id, situation_en, explanation, example_zh, difficulty)
        VALUES (${clusterId}, ${answerWordId}, ${sit.scenario}, ${sit.why}, ${sit.example_zh}, ${sit.difficulty})
        ON CONFLICT (cluster_id, situation_en) DO NOTHING
      `;
    }
  }
